...
function getSecret(): string {
  return ((window as any).__adminSecret || import.meta.env.VITE_ADMIN_SECRET) ?? '';
}

console.log('[Admin API] BASE URL:', BASE);

async function req<T>(path: string, init?: RequestInit): Promise<T> {
  const url = `${BASE}${path}`;
  const SECRET = getSecret();
  console.log('[Admin API] Requesting:', url);
  console.log('[Admin API] SECRET set:', !!SECRET);
  ...
  if (!res.ok) {
    ...
    console.error('[Admin API] Error:', body);
